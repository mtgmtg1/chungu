...
- To create annotations directly from JSON (without using add_highlight/add_callout), call save_annotations with an EmbedPDF AnnotationTransferItem[] array. Use this when you need precise rect positions from view_page or read_job_json. Set merge=false to replace all existing annotations. By default, rect coordinates are expected in PDF user-space (y↑, origin bottom-left) — the tool auto-converts to device-space. When passing existing annotations from read_job_json (already in device-space), set pdf_user_space=false to avoid double conversion.
