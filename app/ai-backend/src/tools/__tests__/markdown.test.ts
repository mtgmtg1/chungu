...
describe('buildMarkdownTools', () => {
  ...
  it('apply_edits 는 working markdown 을 FastAPI 에 저장한다', async () => {
    const tools = buildMarkdownTools({
      jobId: 'job-1',
      selectedFileIndex: 1,
    } as any) as any;
    ...
    assert.equal(savedCalls[0].pageNum, 2);
    assert.ok(savedCalls[0].markdown.includes('Changed details two.'));
  });
