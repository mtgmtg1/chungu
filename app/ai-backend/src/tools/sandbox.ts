...
export function createSandboxTools(context: SandboxContext) {
  ...
  const createSandbox = tool({
    description:
      'Create a new agent sandbox (Kata VM). This tool must be called first before executing code inside the sandbox. ' +
  ...
  const executeInSandbox = tool({
    description:
      'Execute a shell command inside the sandbox. Can run Python scripts, Node.js scripts, git commands, ' +
  ...
  const commitSandboxChanges = tool({
    description:
      'git commit changes in the sandbox workspace. ' +
  ...
  const getSandboxDiff = tool({
    description:
      'Retrieve the git diff of the sandbox workspace. ' +
  ...
  const collectSandboxResults = tool({
    description:
      'Collect result files from the sandbox and upload them to Supabase Storage. ' +
      'Call before destroying the sandbox to permanently save files generated by the agent. ' +
      'Files in agent_output/, extracted/, and annotations/ are collected.',
    inputSchema: z.object({}),
    execute: async () => {
      if (!activeSandboxId) {
        return { error: 'Sandbox has not been created.' };
  ...
  const destroySandbox = tool({
    description:
      'Destroy and clean up the sandbox. Call after the agent\'s work is complete. ' +
      'It is recommended to call collect_sandbox_results before destroying to save results.',
    inputSchema: z.object({}),
    execute: async () => {
      if (!activeSandboxId) {
        return { error: 'Sandbox has not been created.' };
  ...
  const convertDocument = tool({
    description:
      'Convert documents using LibreOffice or Pandoc inside the sandbox. ' +
  ...
  const transcribeAudio = tool({
    description:
      'Convert audio files to text using faster-whisper inside the sandbox. ' +
...
" 2>&1`;
  ...
  const processImage = tool({
    description:
      'Process images using ImageMagick or Pillow inside the sandbox. ' +
...
"`;
  ...
  const getWorkspaceStatus = tool({
    description:
      'Check the sandbox status and resource usage. ' +
