#!/usr/bin/env python3
# [Flow: Step 1 (인증된 사용자 및 요청 검증) -> Step 2 (LLM 스트리밍 시작) -> Step 3 (SSE 형식으로 토큰 반환) -> Step 4 (완료 신호 전송)]
...
async def _token_stream(prompt: str, option: str, command: str | None) -> None:
    """LLM 스트리밍 토큰을 SSE data 라인으로 변환하여 반환한다.
...
async def generate(
    ...
    Returns:
        text/event-stream 형식의 StreamingResponse.
    """
    user, api_key = auth
