...
class TestPrecisionImprovements:
    """방안 1, 2, 3 정밀도 향상 기능 검증 테스트."""

    def test_multiline_parsing_res_list_segmentation(self):
        """[Flow: Step 1 (parsing_res_list의 멀티라인 텍스트 블록 준비)
              -> Step 2 (extract_page_ocr_results_from_layout 실행)
              -> Step 3 (줄바꿈 단위로 N개 분할되어 각 줄별 BBox가 할당되는지 검증)]"""
        layout_by_page = {
            1: {
                "parsing_res_list": [
                    {
                        "block_label": "text",
                        "block_content": "첫 번째 줄입니다.\n두 번째 줄입니다.\n세 번째 줄입니다.",
                        "block_bbox": [100.0, 100.0, 400.0, 250.0],
                    }
                ]
            }
        }
        ...
        # 첫 번째 줄, 두 번째 줄, 세 번째 줄 확인
        assert items[0][0] == "첫 번째 줄입니다."
        assert items[1][0] == "두 번째 줄입니다."
        assert items[2][0] == "세 번째 줄입니다."

        # 각 줄의 y범위가 세로 분할되어 100~150, 150~200, 200~250 범위 근처인지 검증
        y0_line0 = items[0][1][1]
        y1_line0 = items[0][1][3]
        y0_line2 = items[2][1][1]
        y1_line2 = items[2][1][3]

        assert abs(y0_line0 - 100.0) < 1.0
        assert abs(y1_line0 - 150.0) < 1.0
        assert abs(y0_line2 - 200.0) < 1.0
        assert abs(y1_line2 - 250.0) < 1.0

    def test_horizontal_scaling_preserves_height_in_narrow_bbox(self):
        """[Flow: Step 1 (가로 폭은 좁지만 세로 높이는 40pt인 BBox 준비)
              -> Step 2 (add_text_layer_from_ocr로 텍스트 레이어 추가)
              -> Step 3 (Horizontal Scaling으로 폰트 세로 높이가 축소되지 않고 BBox 높이의 75% 이상 유지되는지 검증)]"""
        pdf_bytes = _make_a4_image_pdf()
        page_width = 595.0
        page_height = 842.0

        # 긴 텍스트 "A Very Long Sentence That Is Narrow"가 너비 150pt, 높이 40pt BBox에 들어감
        x0, y0_top, x1, y1_top = 100.0, 200.0, 250.0, 240.0
        bbox_height = y1_top - y0_top  # 40pt
        text = "A Very Long Sentence That Needs Horizontal Scaling"
        ...
        doc = fitz.open(stream=result_bytes, filetype="pdf")
        page = doc[0]
        rects = page.search_for("A Very Long")
        assert len(rects) >= 1

        found = rects[0]
        found_height = found.y1 - found.y0
        # Horizontal scaling이 정상 동작하면 세로 높이가 40pt BBox의 70% 이상을 유지함
        assert found_height >= bbox_height * 0.70, (
            f"Font height {found_height} was shrunk too much (expected >= {bbox_height * 0.70})"
        )
        doc.close()
