...
AIにこのプロンプトと共にユーザーのAPIキーを提供した後、次のようにリクエストします:

> "このPDFから表を抽出してください: [ファイルパス]"
> "これらのカラムを使用してください: 日付、説明、金額、残高"
> "結果をXLSXでダウンロードしてください"

AIがアップロード → 確認 → ポーリング → ダウンロードの全フローを自動的に処理します。
