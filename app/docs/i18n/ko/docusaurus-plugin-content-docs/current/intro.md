...
1. **업로드** — `POST /api/v1/jobs/upload`로 파일을 업로드하고 비용 미리보기를 받습니다 (크레딧 차감 없음)
2. **확인** — `POST /api/v1/jobs/{job_id}/confirm`로 작업을 확인합니다. 크레딧이 차감되고 처리가 시작됩니다
3. **폴링** — `GET /api/v1/jobs/{job_id}`로 `status`가 `done` 또는 `error`가 될 때까지 확인합니다
4. **다운로드** — `GET /api/v1/jobs/{job_id}/download?type=csv_basic|md|xlsx_basic`로 결과를 다운로드합니다

## 시작하기

- PROOF가 처음이신가요? [퀵 스타트](./quickstart) 가이드를 읽어보세요
- API 키가 필요하신가요? [개발자 포털](pathname:///developer)을 방문하세요
