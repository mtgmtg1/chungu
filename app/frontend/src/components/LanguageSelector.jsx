  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="font-medium uppercase">{current.value}</span>
      </button>
      {open && (
        <>
          <div
          ...
          />
          <div className="absolute right-0 top-full mt-2 w-40 bg-surface rounded-xl shadow-lg border border-outline-variant z-50 py-1">
            {LANGUAGE_OPTIONS.map((option) => (
              <button
                key={option.value}
                onClick={() => handleSelect(option.value)}
                className={`w-full flex items-center gap-3 px-4 py-2 text-sm text-left transition-colors ${
                  language === option.value ? 'bg-primary-container/10 text-primary font-medium' : 'text-on-surface hover:bg-surface-container-high'
                }`}
              >
