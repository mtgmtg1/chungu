...
export default function DashboardPage() {
  ...
  function formatDate(dateStr) {
    if (!dateStr) return '-'
    return new Date(dateStr).toLocaleString(i18n.language, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
  }
  ...
  return (
    <SidebarLayout title={t('page:dashboard.title')} subtitle={t('page:dashboard.subtitle')}>
      {error && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-gutter mb-stack-lg">
        <div className="glass-panel p-6 rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <p className="font-label-sm text-label-sm text-on-surface-variant">{t('page:dashboard.points')}</p>
          ...
          <Link to="/payment" className="mt-3 inline-flex items-center gap-1 text-primary text-sm font-medium hover:underline">
            <CreditCard size={14} /> {t('page:dashboard.recharge')}
          </Link>
        </div>

        <div className="glass-panel p-6 rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <p className="font-label-sm text-label-sm text-on-surface-variant">{t('page:dashboard.totalJobs')}</p>
            <div className="p-2 bg-primary/10 rounded-lg text-primary">
              <span className="material-symbols-outlined">task</span>
            </div>
          </div>
          <p className="text-3xl font-bold text-on-surface">{stats.total}</p>
          <p className="mt-3 text-sm text-on-surface-variant">{t('page:dashboard.totalJobsDesc')}</p>
        </div>

        <div className="glass-panel p-6 rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <p className="font-label-sm text-label-sm text-on-surface-variant">{t('page:dashboard.completed')}</p>
            <div className="p-2 bg-green-50 rounded-lg text-green-600">
              <span className="material-symbols-outlined">check_circle</span>
            </div>
          </div>
          <p className="text-3xl font-bold text-on-surface">{stats.done}</p>
          <p className="mt-3 text-sm text-on-surface-variant">{t('page:dashboard.completedDesc')}</p>
        </div>

        <div className="glass-panel p-6 rounded-2xl">
          <div className="flex items-center justify-between mb-4">
            <p className="font-label-sm text-label-sm text-on-surface-variant">{t('page:dashboard.processedPages')}</p>
          ...
          <p className="mt-3 text-sm text-on-surface-variant">{t('page:dashboard.processedPagesDesc')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-gutter mb-stack-lg">
        ...
        <div className="lg:col-span-2 glass-panel rounded-2xl p-6">
          <h3 className="font-headline-md text-headline-md text-on-surface mb-6">{t('page:dashboard.statusBreakdown')}</h3>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              { key: 'active', label: t('page:dashboard.active'), value: stats.active },
              { key: 'done', label: t('page:dashboard.completed'), value: stats.done },
              { key: 'error', label: t('page:dashboard.failed'), value: stats.error },
              { key: 'pending', label: t('page:dashboard.pending'), value: jobs.filter((j) => j.status === 'pending').length },
            ].map((item) => (
              ...
              return (
                <div key={key}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="text-on-surface-variant">{statusLabel(key)}</span>
        ...
        <div className="glass-panel rounded-2xl p-6">
          <h3 className="font-headline-md text-headline-md text-on-surface mb-6">{t('page:dashboard.quickActions')}</h3>
      ...
      <div className="glass-panel rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-outline-variant flex justify-between items-center">
          <h3 className="font-headline-md text-headline-md text-on-surface">{t('page:dashboard.recentJobs')}</h3>
          <Link to="/jobs" className="text-primary text-sm font-medium hover:underline flex items-center gap-1">
            {t('page:dashboard.viewAll')} <ArrowRight size={14} />
          </Link>
        </div>
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left">
            <thead className="bg-surface-container-low/50 text-on-surface-variant font-label-sm text-label-sm uppercase tracking-wider">
              <tr>
                <th className="px-gutter py-4">{t('page:dashboard.fileName')}</th>
                <th className="px-gutter py-4">{t('page:dashboard.status')}</th>
                <th className="px-gutter py-4">{t('page:dashboard.date')}</th>
                <th className="px-gutter py-4 text-right">{t('page:dashboard.action')}</th>
              </tr>
            </thead>
                    ...
                    <tr key={j.job_id} className="hover:bg-surface-container/30 transition-colors">
                      ...
                      <td className="px-gutter py-4 text-right">
                        {j.status === 'done' ? (
                          ...
                          >
                            <Eye size={14} /> {t('page:dashboard.view')}
                          </Link>
                        ) : (
      ...
      <div className="mt-stack-lg grid grid-cols-1 md:grid-cols-3 gap-gutter">
        <div className="col-span-1 md:col-span-2 glass-surface p-gutter rounded-2xl border border-primary/10 flex items-start gap-4">
          ...
          <div>
            <h4 className="font-headline-md text-headline-md text-primary mb-2">{t('page:dashboard.apiPromoTitle')}</h4>
            ...
            <Link to="/developer" className="mt-4 text-primary font-body-md text-body-md font-bold hover:underline inline-flex items-center gap-1">
              {t('page:dashboard.apiPromoLink')} <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </div>
