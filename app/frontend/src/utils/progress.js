// [Flow: Step 1 (실제 진행률 계산) -> Step 2 (백엔드 처리 시작 시점 또는 job.created_at 기준 경과 시간 계산) -> Step 3 (시간 기반 추정 진행률 계산, 50배 느림) -> Step 4 (80% cap 및 실제값 우선으로 합산)]
...
const TIME_PROGRESS_SLOWDOWN = 50; // 전체 페이지 수 * 50 초에 100%에 도달하도록 기존 대비 5배 느리게
 ...
 * 전체 작업량의 50배 시간(초)에 100%에 도달하며, maxTimePct(기본 80%)까지만 올라간다.
 * @param {object} job - API에서 받은 job 객체
